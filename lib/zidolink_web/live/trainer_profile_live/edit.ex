defmodule ZidolinkWeb.TrainerProfileLive.Edit do
      ...
      <div class="mx-auto max-w-lg">
        ...
        <.form for={@form} id="trainer_profile_form" phx-submit="save" phx-change="validate">
          ...
          <.input
            ...
            type="text"
            label="Specialties (comma separated, e.g. weight loss, strength training)"
          />
          ...
          <div class="mt-4">
            ...
            <p :if={@form[:location_lat].value not in [nil, ""]} class="text-sm text-base-content/80 mt-2">
  ...
  def handle_event("validate", %{"trainer_profile" => params}, socket) do
    changeset =
      ...
      |> TrainerProfile.changeset(normalize_specialties(params))
  ...
  def handle_event("save", %{"trainer_profile" => params}, socket) do
    ...
    params = normalize_specialties(params) |> Map.put("profile_completed", true)
  ...
  defp normalize_specialties(params) do
    case params["specialties"] do
      list when is_list(list) -> params
      nil -> Map.put(params, "specialties", [])
      "" -> Map.put(params, "specialties", [])
      str -> Map.put(params, "specialties", str |> String.split(",") |> Enum.map(&String.trim/1) |> Enum.reject(&(&1 == "")))
    end
  end
